...
Example:
Checkpoint, SessionList = action_db_connection(cursor,
connection, Checkpoint, SessionLists)
"""
...
def action_db_connection(cursor, connection, session, session_list):
    ...
    Example:
    session, session_list = action_db_connection(cursor, connection,
...
def action_pick_categorie(
    ...
    Example:
    session, food_item, subst_item, session_list = action_pick_categorie(
...
def action_pick_food(
        cursor, connection, session, food_item, subst_item,
        session_list):
    """ Display all distint food item name of the table Food
    in Pur_Beurre database.
    Make the user do a choice between them inputing a number.
    Select the best substitute of the food item in the table Food
    and displays it.
    ...
    while not session.pick_food:
        ...
        if food_select.lower() == 'q':
            ...
            if food_item.nutriscore == subst_item.nutriscore:
                print(
                    "\nDésolé mais nous n'avons trouver aucun substitut\
                    avec un meilleur nutriscore.\nA nutriscore équivalent\
                    nous vous proposons cependant le produit suivant.")
            print(
    ...
    while not session.save:
        ...
        if save_select.lower() == "o":
            session.save_request(
                cursor, connection, subst_item.id,
                subst_item.cat_id, food_item.id)
            print("Votre recherche a bien été sauvegardée.")

        elif save_select.lower() == "n":
            print(
                "\nRetour au menu principal. Votre recherche\
    ...
    while not session.history:
        ...
        for element in session_list.history:
            print(
                ...
                "\nNutriscore de l'aliment d'origine: ",
                element.origin_nutriscore,
                "\nDescription de l'aliment d'origine: ",
                element.origin_description,
